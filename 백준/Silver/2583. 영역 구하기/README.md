...
### ✅ 접근 방법
이 문제는 그래프를 홣용하여 푸는 문제로, 이번 해결에서는 BFS 를 통해 비어있는 영역을 탐색하였다. (섬의 개수 구하기 문제랑 비슷하다)
다른 그래프문제들과 마찬가지로, visited 배열과 map 배열을 활용하여 해결한다.
BFS 의 경우 큐를 통해 해결해야한다. 조심해야 하는 부분은 BFS 시 큐를 정수형배열을 저장하는 형태로 선언해야한다. --> 좌표값이므로
