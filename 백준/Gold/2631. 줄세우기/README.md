...
### ✅ 접근 방법
이 문제는 DP를 활용하여 푸는 문제로, 최장증가수열(LIS) 를 기억하면 쉽게 풀 수 있다.
LIS 를 사용한다면, 우리는 가장 최대의 길이를 가지는 수열의 요소개수가 몇개인지 알 수 있다.
즉 최장증가수열의 길이만큼을 원래 길이에서 빼준만큼을 우리가 옮겨야 하는 것과 같다.

### ✅ 알게된 내용
처음에는 2차원 DP 배열을 만들고 점화식을 찾아보려했지만, 수업시간에 배운 LIS 를 활용하는 문제라는 것을 알게 된 뒤, 많은 것을 생각하게 된 문제이다. 
